...
/// The page a frosted card sits on, owned across frames. The rim smears its taps wide, so its
/// copy is half resolution too: a quarter of the memory of holding the snapshot itself.
pub(crate) struct Page {
    sharp: skia_safe::Image,
    blurred: skia_safe::Image,
}
...
/// Release the rim shaders' hold on page and cover textures.
pub(crate) fn release() {
    super::card_material::release();
}
...
/// Where the page image lands in local space. Uses both frame dimensions for nonuniform
/// display scaling, and cancels modal-rise translation so it stays registered with the page.
fn page_rect(f: &Frame<'_>) -> Rect {
    let m = f.canvas.local_to_device_as_3x3();
...
fn draw_card_backdrop(canvas: &Canvas, page: &skia_safe::Image, dst: Rect, rr: RRect) {
    canvas.save();
    canvas.clip_rrect(rr, None, Some(true));
    canvas.draw_image_rect_with_sampling_options(page, None, dst, super::linear(), &theme::layer());
    canvas.restore();
}
...
pub(crate) fn glass_card(f: &Frame<'_>, rect: Rect, corner: f32) {
    ...
    let dst = page_rect(f);
    draw_card_backdrop(canvas, page.blurred, dst, rr);
    draw_face(canvas, rr, k, page.sharp, dst);
}

/// The translucent face over whatever backdrop the caller has already laid down.
/// `page` is the sharp image under the card, laid over `dst`, for the rim to take its colour from.
fn draw_face(canvas: &Canvas, rr: RRect, k: f32, page: &skia_safe::Image, dst: Rect) {
    canvas.draw_rrect(rr, &theme::fill(frosted_face()));
    // theme::panel would re-opacify the translucent face.
    card_material::draw(canvas, rr, k, page, dst);
}
...
pub(crate) fn frost_over_art(canvas: &Canvas, img: &skia_safe::Image, art: Rect, window: Rect, k: f32) -> bool {
    ...
    draw_face(canvas, RRect::new_rect(window), k, img, art);
    true
}
